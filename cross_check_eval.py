...
def main():
    ...
    parser.add_argument("--engine", type=str, default="stockfish", help="path to stockfish", required=False)
    parser.add_argument("--data", type=str, help="path to a .bin or .binpack dataset", required=True)
    parser.add_argument(
        "--checkpoint",
        type=str,
    ...
    parser.add_argument(
        "--count", type=int, default=100, help="number of datapoints to process", required=False
    )
    parser.add_argument(
        "--model", type=str, help="name of the model to test", required=True
    )
    parser.add_argument(
        "--batch", type=int, default=1000, help="batch size", required=False
    )
    ...
    model = models.nets[args.model](feature_set)
    model.eval()
    model.cuda()
    ...
    while done < args.count:
        ...
        engine_evals += eval_engine_batch(args.engine, args.net, fens) # TODO: ten net do sprawdzenia
