...
def position_to_fen(pos):
    ...
    # Extract piece placement from board (rows 2-9, cols 1-8)
    fen_rows = []
    for rank in range(8):
    ...
    # Side to move (we track whose perspective we're in)
    # In sunfish, when it's white's turn, uppercase pieces are ours
    side = 'w'  # We'll handle color in evaluation

    # Castling rights
    castling = ''
    if pos.wc[1]: castling += 'K'  # White kingside
    if pos.wc[0]: castling += 'Q'  # White queenside
    if pos.bc[1]: castling += 'k'  # Black kingside
    if pos.bc[0]: castling += 'q'  # Black queenside
    ...
    # Halfmove clock and fullmove number (use defaults)
